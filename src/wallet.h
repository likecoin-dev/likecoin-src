...
public:
    ...
    int64_t GetLikeDebit(const CTxIn& txin) const;
    int64_t GetLikeCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nLIKE, int64_t& nLike) const
    {
        nLIKE = 0;
        nLike = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nLike += GetLikeCredit(txout);
            } else
                nLIKE += GetCredit(txout);
            if (!MoneyRange(nLIKE)
                || !MoneyRange(nLike))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableLikeCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableLikeCreditCached;
    
    mutable int64_t nCredLIKECached;
    mutable int64_t nCredLikeCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableLikeCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredLIKE, int64_t& nCredLike, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredLIKE = nCredLike = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredLIKECached = 0;
            nCredLikeCached = 0;
            pwallet->GetCredit(*this, nCredLIKECached, nCredLikeCached);
            fCreditSplitCached = true;
        };
        
        nCredLIKE = nCredLIKECached;
        nCredLike = nCredLikeCached;
        return true;
    }
    ...
    int64_t GetAvailableLikeCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableLikeCreditCached)
            return nAvailableLikeCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableLikeCredit() : value out of range");
            };
        };
        
        nAvailableLikeCreditCached = nCredit;
        fAvailableLikeCreditCached = true;
        return nCredit;
    };
