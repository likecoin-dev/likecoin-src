...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to likecoind / RPC client
            std::string strUsage = _("LIKEcoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  likecoind [options]                     " + "\n" +
                  "  likecoind [options] <command> [params]  " + _("Send command to -server or likecoind") + "\n" +
                  "  likecoind [options] help                " + _("List commands") + "\n" +
                  "  likecoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "likecoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect likecoind signal handlers
    noui_connect();
