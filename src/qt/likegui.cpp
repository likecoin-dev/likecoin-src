// Copyright (c) 2014 The LIKEcoin developers
...
LikeGUI::LikeGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new LikeBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("LikeClient") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/like"));
    setWindowIcon(QIcon(":icons/like"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/like/index.html");
#else
    QFile html("/opt/like/index.html");
#endif
...
void LikeGUI::pageLoaded(bool ok)
{
...
void LikeGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void LikeGUI::urlClicked(const QUrl & link)
{
...
void LikeGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/like"), tr("&About LIKEcoin"), this);
    aboutAction->setToolTip(tr("Show information about LIKEcoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for LIKEcoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/like"), tr("&Show / Hide"), this);
...
void LikeGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("LikeClient") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/like_testnet"));
            setWindowIcon(QIcon(":icons/like_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/like_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("LikeClient client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/like_testnet"));
...
void LikeGUI::setWalletModel(WalletModel *walletModel)
{
...
void LikeGUI::setMessageModel(MessageModel *messageModel)
{
...
void LikeGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("LikeClient client"));
    trayIcon->setIcon(QIcon(":/icons/like"));
...
#ifndef Q_OS_MAC
void LikeGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void LikeGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to LIKEcoin network", "", count));
}

void LikeGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void LikeGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void LikeGUI::changeEvent(QEvent *e)
{
...
void LikeGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void LikeGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, nFeeRequired));
...
void LikeGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void LikeGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void LikeGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void LikeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void LikeGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void LikeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid LIKEcoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void LikeGUI::handleURI(QString strURI)
{
...
void LikeGUI::setEncryptionStatus(int status)
{
...
void LikeGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void LikeGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void LikeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LikeGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void LikeGUI::updateStakingIcon()
{
