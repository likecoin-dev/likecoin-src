...
#include "likebridge.moc"

LikeBridge::LikeBridge(LikeGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void LikeBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void LikeBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void LikeBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void LikeBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void LikeBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void LikeBridge::populateOptions()
{
...
// Transactions
bool LikeBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void LikeBridge::clearRecipients()
{
    recipients.clear();
}

bool LikeBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 LIKE, 1 Like
        switch(rcp.txnTypeInd)
        {
            case TXT_LIKE_TO_LIKE:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_LIKE_TO_ANON:
                formatted.append(tr("<b>%1</b> to LIKE %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> LIKE, ring size %2 to LIKE %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_LIKE:
                formatted.append(tr("<b>%1</b> LIKE, ring size %2 to LIKE %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsLikeBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your like balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIKE, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void LikeBridge::openCoinControl()
{
...
void LikeBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void LikeBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap LikeBridge::listAnonOutputs()
{
...
void LikeBridge::populateTransactionTable()
{
...
void LikeBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void LikeBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString LikeBridge::transactionDetails(QString txid)
{
...
// Addresses
void LikeBridge::populateAddressTable()
{
...
void LikeBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void LikeBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString LikeBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString LikeBridge::lastAddressError()
{
    QString sError;
...
QString LikeBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void LikeBridge::updateAddressLabel(QString address, QString label)
{
...
bool LikeBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool LikeBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void LikeBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void LikeBridge::appendMessage(int row)
{
...
void LikeBridge::populateMessageTable()
{
...
void LikeBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool LikeBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool LikeBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString LikeBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString LikeBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool LikeBridge::setPubKey(QString address, QString pubkey)
{
...
bool LikeBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString LikeBridge::createGroupChat(QString label)
{
...
QString LikeBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList LikeBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void LikeBridge::connectSignals()
{
...
QString LikeBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap LikeBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap LikeBridge::listLatestBlocks()
{
...
QVariantMap LikeBridge::findBlock(QString searchID)
{
...
QVariantMap LikeBridge::blockDetails(QString blkHash)
{
...
QVariantMap LikeBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap LikeBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap LikeBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap LikeBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap LikeBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap LikeBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap LikeBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap LikeBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap LikeBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Likecoin prefix.");
            return result;
        }
...
QVariantMap LikeBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap LikeBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap LikeBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
