...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return LikeGUI::tr("Received with");
    case RecvFromOther:
        return LikeGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return LikeGUI::tr("Sent to");
    case SendToSelf:
        return LikeGUI::tr("Payment to yourself");
    case Generated:
        return LikeGUI::tr("Mined");
    case RecvLike:
        return LikeGUI::tr("Received like");
    case SendLike:
        return LikeGUI::tr("Sent like");
    case Other:
        return LikeGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvLike:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendLike:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredLIKE, nCredLike;
    wtx.GetCredit(nCredLIKE, nCredLike, true);
    int64_t nCredit = nCredLIKE + nCredLike;
    ...
    {
        if (nNet > 0 && nCredLike > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvLike, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendLike, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvLike;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv like
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendLike;
                    sub.address = CBitcoinAddress(ckidD).ToString();
